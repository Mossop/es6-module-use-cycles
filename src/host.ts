...
export class ModuleHost {
  ...
  public parseEntrypoint(entrypoint: string): SourceTextModuleRecord {
    let sourceText = fs.readFileSync(entrypoint, { encoding: "utf8" });
    return this.topLevelModuleEvaluation(sourceText, entrypoint);
  }

  public topLevelModuleEvaluation(sourceText: string, targetPath: string): SourceTextModuleRecord {
  ...
  public parseModule(sourceText: string, targetPath: string): SourceTextModuleRecord {
    ...
    let record: SourceTextModuleRecord | null = null;
      ...
      create: (context: Rule.RuleContext): Rule.RuleListener => {
        return {
          Program: (node: ESTree.Program): void => {
            record = new SourceTextModuleRecord(this, targetPath, node);
            this.moduleRecords.set(targetPath, record);

            let parser = createParser(record, context);
    ...
    if (!record) {
      internalError("ModuleRecord was not created during parsing..", targetPath, null);
    }
